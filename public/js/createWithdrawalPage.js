/**
 * Creates a withdrawal page in the DOM without using document.write
 * This prevents page reloads that interrupt AJAX requests
 * @param {number} availableBalance - Available balance for withdrawal
 */
function createWithdrawalPage(availableBalance) {
    // Save the document title and update it
    const originalTitle = document.title;
    document.title = "Withdraw USDT (TRC20)";
    ...
    let styleSheetsExist = false;
    existingStyles.forEach(link => {
        if(link.href.includes('tailwindcss') || link.href.includes('font-awesome')) {
            styleSheetsExist = true;
        }
    });
    ...
    if(!styleSheetsExist) {
        ...
        `;
        document.head.insertAdjacentHTML('beforeend', styleLinks);
    }
    ...
    setupWithdrawalFormHandlers(availableBalance);
}

/**
 * Sets up event handlers for the withdrawal form
 * @param {number} availableBalance - Available balance for withdrawal
 */
function setupWithdrawalFormHandlers(availableBalance) {
